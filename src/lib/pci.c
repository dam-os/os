...
uint32_t pci_read_word(uint8_t bus, uint8_t device, uint8_t function, uint8_t offset) {
    volatile uint32_t *addr = (volatile uint32_t *)(PCI_CONFIG_BASE +
...
void enumerate_pci() {
    for (uint8_t bus = 0; bus < 255; bus++) {         // Scan all PCI buses (0-255)
        for (uint8_t device = 0; device < 32; device++) {  // Each bus has 32 possible devices
